...
object Archive {
   ...
   */
  def zipReader(in: InputStream, chunkSize: Int): Source[(ZipArchiveMetadata, ByteString), NotUsed] =
    Source.fromGraph(new ZipSource(in, chunkSize))
  def zipReader(in: InputStream): Source[(ZipArchiveMetadata, ByteString), NotUsed] =
    Source.fromGraph(new ZipSource(in, 8192))
